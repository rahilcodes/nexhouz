...
import { Sparkles, ShieldCheck, Heart, MapPin, ArrowRight, Building, Star, TrendingUp, Filter, Search } from "lucide-react";
import Navbar from "@/components/layout/Navbar";
import Footer from "@/components/layout/Footer";
import { properties } from "@/data/properties";

const newLaunchProperties = properties.filter(p => p.possession === "Under Construction");
          ...
          <div className="relative max-w-7xl mx-auto px-6">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="max-w-3xl">
              <div className="inline-flex items-center gap-2 px-3 py-1 bg-brand-red/15 border border-brand-red/30 rounded-full mb-5">
                ...
                <span className="text-[9px] font-extrabold tracking-[0.2em] uppercase text-brand-red">Freshly Launched</span>
              </div>
        ...
        <section className="py-20">
          <div className="max-w-7xl mx-auto px-6">
            <div className="flex items-end justify-between mb-10">
              <div>
                <p className="text-[10px] font-extrabold uppercase tracking-widest text-brand-red mb-2">Curated New Launches</p>
                  ...
                  <motion.div key={property.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: idx * 0.1 }} className="bg-white border border-gray-100 rounded-3xl overflow-hidden shadow-sm hover:shadow-lg hover:border-brand-red/15 transition-all group">
                    <Link href={`/properties/${property.slug}`} className="block relative aspect-[4/3] overflow-hidden bg-gray-100">
                      ...
                      <div className="absolute top-3 left-3 flex items-center gap-1.5">
                        <span className="bg-brand-red text-white text-[9px] font-extrabold uppercase tracking-wider px-2.5 py-1 rounded-full flex items-center gap-1">
